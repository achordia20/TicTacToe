...
public class TicTacToeAI {
	...
	private int getBoardScore() {
		int score = 0;
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(1, 2, 3)));
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(4, 5, 6)));
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(7, 8, 9)));
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(1, 4, 7)));
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(2, 5, 8)));
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(3, 6, 9)));
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(1, 5, 9)));
		score += calculateScore(new ArrayList<Integer>(Arrays.asList(3, 5, 7)));
		return score;
	}
	
	private int calculateScore(ArrayList<Integer> positions) {
		int myMoveTypeCount = 0, theirMoveCount = 0, emptyMoveCount = 0;
		for (Integer pos : positions) {
			MoveType m = board.atPos(pos);
			if (myMoveType == m)
				myMoveTypeCount++;
			else if (theirMoveType == m)
				theirMoveCount++;
			else
				emptyMoveCount++;
		}
		
		if (myMoveTypeCount > 0 && theirMoveCount > 0)
			return 0;
		
		if (emptyMoveCount == positions.size())
			return 0;
		
		if (myMoveTypeCount > 0)
			return (int) Math.pow(10, myMoveTypeCount);
		
		if (theirMoveCount > 0)
			return -1 * (int)Math.pow(10, theirMoveCount);
